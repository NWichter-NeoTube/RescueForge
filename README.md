...
Fields: file (DWG/DXF, max 50 MB), language (optional: "en" or "de")
```

Response `200`: `{ "job_id": "uuid", "filename": "plan.dxf", "status": "pending" }`

Errors: `400` invalid format, `413` file too large, `429` rate limited (20 req/60s per IP)

#### Batch Upload (POST `/api/upload/batch`)

Up to 10 DWG/DXF files. Returns `batch_id`, list of `jobs`, count, and `skipped` filenames.

#### Job Status (GET `/api/jobs/{job_id}`)

Status values: `pending` → `converting` → `parsing` → `detecting_rooms` → `classifying` → `generating` → `exporting` → `completed` (or `failed`)

Response includes `progress` (0.0–1.0) and `message`. On completion: `result_svg` and `result_pdf` paths.

#### Room Update (PUT `/api/jobs/{job_id}/rooms`)

```json
[{ "id": 1, "room_type": "office", "label": "Büro 1" }, { "id": 2, "room_type": "corridor" }]
```

Regenerates orientation plan, cover sheet, and situation plan.

#### WebSocket (WS `/api/ws/{job_id}`)

Real-time progress updates (500ms polling). Message types: `progress`, `complete`, `error`.

#### Room Types

`office`, `corridor`, `stairwell`, `elevator`, `bathroom`, `kitchen`, `storage`, `technical`, `server_room`, `garage`, `lobby`, `conference`, `residential`, `bedroom`, `living_room`, `balcony`, `unknown`
...
├── doors: [DoorInfo]           # position, width, angle
...
DXF files use no standardized layer names. The pipeline uses pattern matching:

| Pattern | Classification | Action |
|---------|---------------|--------|
| wall, wand, mauer, mur, tuong_xay | Wall | Keep |
| door, tür, tuer, porte, puerta | Door | Keep |
| stair, treppe, cau thang, escalera | Stairs | Keep |
| brandwand, fire_wall, fw- | Fire wall | Keep (RAL 3000) |
| brandschutz, t30, t60, t90, ei30 | Fire door | Keep (red) |
| window, fenster, glazing | Window | Keep |
| sprinkler, spk, löschanlage | Sprinkler | Keep |
| furn, möbel | Furniture | Remove |
| dim, mass, anno | Dimension | Remove |
| hatch, schraffur | Hatch | Remove |
| (unknown, thick ≥0.3mm) | Check lineweight | → Wall |
...
- **DWGShare.com** — Free DWG floor plans (apartments, offices, hotels, schools, factories)
- **FloorPlanCAD** — 15,000+ real CAD floor plans, 30 categories (CC BY-NC 4.0) — [floorplancad.github.io](https://floorplancad.github.io/)
- **ArchCAD-400K** — 5,538 drawings, 413,062 annotated sections — [huggingface.co/datasets/jackluoluo/ArchCAD](https://huggingface.co/datasets/jackluoluo/ArchCAD)
- **CubiCasa5K** — 5,000 floor plans, 80+ categories — [github.com/CubiCasa/CubiCasa5k](https://github.com/CubiCasa/CubiCasa5k)

## FKS Guideline Reference

Output plans follow the [FKS Richtlinie Orientierungspläne](https://docs.feukos.ch/richtlinie-orientierungsplane/) (valid from 01.08.2024):

- **Plan types:** Cover sheet (Deckblatt), floor plans (Geschosspläne), situation plan (Situationsplan)
- **Format:** A4 or A3, max scale 1:300, all plans same scale and orientation
- **Color coding:** Dark green (vertical escape routes), light green (horizontal), red (fire alarm / danger), blue (sprinkler)
- **Symbols:** SES standard — smoke detectors, manual call points, gas detectors, alarm devices (min. 3x3mm)
- **Referenced standards:** VKF 20-15 (BMA), VKF 19-15 (SPA), SES Richtlinie Brandmeldeanlagen
